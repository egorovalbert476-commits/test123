...
const contractScreen = document.querySelector('.contract-screen');
...
const SUPABASE_URL = 'https://xhxeuklfmlkzuftmymwx.supabase.co';
const SUPABASE_ANON_KEY = 'sb_publishable_VTGGHT2VJ-YWIqqaONh81A_wiJ1PqBv';
...
const DEFAULT_CASE_SECTIONS = [
  {
    ...
    cases: [
      { id: 'blue-case', name: 'Синий Кейс', price: 2490, image: resolveAssetPath('img/blue.png') },
      { id: 'green-case', name: 'Зеленый Кейс', price: 2990, image: resolveAssetPath('img/green.png') },
      { id: 'red-case', name: 'Красный Кейс', price: 3490, image: resolveAssetPath('img/red.png') },
      { id: 'pink-case', name: 'Розовый Кейс', price: 4290, image: resolveAssetPath('img/purple.png') },
      { id: 'rainbow-case', name: 'Радужный Кейс', price: 6990, image: resolveAssetPath('img/rgb.png') }
    ]
  }
];
        ...
        ${(section.cases || []).map((item) => `
          <article class="case-card ${item.rarity ? `case-card--${item.rarity}` : ''}">
            <div class="case-card__image-wrap">
              <img src="${resolveAssetPath(item.image || 'img/blue.png')}" alt="${item.name}" />
            </div>
            <div class="case-card__name">${item.name}</div>
            <button type="button" data-case-id="${item.id || ''}" data-case-name="${item.name}" data-case-price="${formatPrice(Number(item.price || 0))}" data-case-payout-chance="${Number(item.payoutChance ?? 25)}" data-case-image="${resolveAssetPath(item.image || 'img/blue.png')}">${formatPrice(Number(item.price || 0))}</button>
          </article>
        `).join('')}
      </div>
    </section>
  ...
  const remoteSections = groups.map((group) => ({
    ...
    cases: (remoteCases || [])
      ...
      .map((item) => ({
        ...
        image: item.image_url || resolveAssetPath('img/blue.png')
      }))
  }));
...
const CASE_SKINS = [
  {
    name: 'Default Skin',
    image: resolveAssetPath('img/blue.png'),
    rarity: 'gray',
    price: 0
  },
  {
  ...
  {
    name: 'AWP',
    image: resolveAssetPath('img/green.png'),
    rarity: 'green'
  },
  {
    name: 'M4A1-S',
    image: resolveAssetPath('img/red.png'),
    rarity: 'red'
  },
  {
    name: 'USP-S',
    image: resolveAssetPath('img/purple.png'),
    rarity: 'purple'
  },
  {
    name: 'Glock-18',
    image: resolveAssetPath('img/rgb.png'),
    rarity: 'pink'
  },
  {
    name: 'Desert Eagle',
    image: resolveAssetPath('img/logo.png'),
    rarity: 'gold'
  }
];
...
function renderCaseContents(skins) {
  ...
  list.innerHTML = (skins || []).length ? skins.map((item) => `
...
function rollCase() {
  ...
  const playerChanceEnabled = isPayoutSystemEnabled(caseUser);
  ...
  const playerPayoutChance = Math.max(0, Math.min(100, Number(caseUser?.casePayoutChance ?? 25)));
  const casePayoutChance = Math.max(0, Math.min(100, Number(activeCasePayoutChance ?? 25)));
  const effectivePayoutChance = playerChanceEnabled ? playerPayoutChance : casePayoutChance;
  if (playerChanceEnabled && effectivePayoutChance === 100 && !profitableIndexes.length) {
    isRolling = false;
  ...
  const highestValueIndex = items.reduce((bestIndex, item, index) => (
    Number(item.skin.price || 0) > Number(items[bestIndex]?.skin.price || 0) ? index : bestIndex
  ), 0);
  const winnerPool = shouldPayOut
    ? [highestValueIndex]
    : playerChanceEnabled && effectivePayoutChance === 100
      ? profitableIndexes
...
function normalizeUser(user) {
  ...
  return {
    ...
    contractPayoutChance: Number(user.contractPayoutChance ?? 100),
...
async function refreshActiveUserFromSupabase() {
  ...
  const { data, error } = await supabaseClient
    .from('profiles')
    .select('username, role, balance, inventory, opened_cases, upgrades, contracts, payout_system_enabled, case_payout_chance, upgrade_payout_chance, contract_payout_chance, best_drop')
    .eq('id', user.id)
    .maybeSingle();
  ...
  saveActiveUser({
    ...
    contracts: data.contracts ?? user.contracts,
    ...
    contractPayoutChance: data.contract_payout_chance ?? user.contractPayoutChance,
...
function getSkinImageByRarity(rarity) {
  const map = {
    gray: resolveAssetPath('img/blue.png'),
    blue: resolveAssetPath('img/blue.png'),
    green: resolveAssetPath('img/green.png'),
    red: resolveAssetPath('img/red.png'),
    purple: resolveAssetPath('img/purple.png'),
    pink: resolveAssetPath('img/rgb.png'),
    gold: resolveAssetPath('img/logo.png')
  };

  return map[rarity] || resolveAssetPath('img/blue.png');
}
    ...
    if (bestDrop && Number(bestDrop.price || 0) > 0) {
      bestDropNode.innerHTML = `
        ...
        <div class="profile-screen__drop-icon">
          <img src="${bestDrop.image || getSkinImageByRarity(bestDrop.rarity || 'blue')}" alt="${bestDrop.name}" />
        </div>
      `;
    } else {
      bestDropNode.innerHTML = `
...
const contractState = { selectedIndexes: new Set() };

function getContractItems() {
  return getInventoryByUser(getActiveUser())
    .map((item, inventoryIndex) => ({ ...item, inventoryIndex }))
    .filter((item) => !item.pendingWithdraw);
}

function updateContractSummary() {
  const items = getContractItems();
  const selected = Array.from(contractState.selectedIndexes)
    .filter((index) => index < items.length)
    .map((index) => items[index]);
  const total = selected.reduce((sum, item) => sum + Number(item.price || 0), 0);
  const countNode = document.getElementById('contractSelectedCount');
  const valueNode = document.getElementById('contractInputValue');
  const maxNode = document.getElementById('contractMaxReward');
  const submit = document.getElementById('contractSubmit');

  if (countNode) countNode.textContent = selected.length;
  if (valueNode) valueNode.textContent = formatPrice(total);
  if (maxNode) maxNode.textContent = `до ${formatPrice(total * 3)}`;
  if (submit) submit.disabled = selected.length < 3 || selected.length > 10;
}

function renderContractItems() {
  const list = document.getElementById('contractItems');
  if (!list) return;

  const items = getContractItems();
  const itemMarkup = items.map((item, index) => `
    <button type="button" class="contract-item ${contractState.selectedIndexes.has(index) ? 'is-selected' : ''} ${item.rarity || 'blue'}" data-contract-index="${index}" data-contract-position="${index + 1}">
      <img src="${item.image || getSkinImageByRarity(item.rarity || 'blue')}" alt="${item.name}" />
      <span class="contract-item__name">${item.name}</span>
      <span class="contract-item__price">${formatPrice(item.price)}</span>
    </button>
  `).join('');
  const emptySlots = Array.from({ length: Math.max(0, 10 - items.length) }, (_, index) => `
    <button type="button" class="contract-item contract-item--empty" data-contract-position="${items.length + index + 1}" disabled>
      <span>${items.length + index + 1}</span>
    </button>
  `).join('');
  list.innerHTML = itemMarkup + emptySlots;
  updateContractSummary();
}

async function openContractScreen() {
  await refreshActiveUserFromSupabase();
  const user = getActiveUser();
  if (!user) {
    openAuthModal();
  ...
  contractState.selectedIndexes.clear();
  renderContractItems();
  const status = document.getElementById('contractStatus');
  if (status) status.textContent = '';
  const rewardPreview = document.getElementById('contractRewardPreview');
  const rewardName = document.getElementById('contractRewardName');
  if (rewardPreview) rewardPreview.textContent = '?';
  if (rewardName) rewardName.textContent = 'Будет получен';
  body.classList.add('contract-screen-open');
  if (contractScreen) {
    contractScreen.hidden = false;
    contractScreen.setAttribute('aria-hidden', 'false');
  }
}

function closeContractScreen() {
  body.classList.remove('contract-screen-open');
  if (contractScreen) {
    contractScreen.hidden = true;
    contractScreen.setAttribute('aria-hidden', 'true');
  }
}

document.getElementById('contractItems')?.addEventListener('click', (event) => {
  const itemButton = event.target.closest('[data-contract-index]');
  if (!itemButton) return;
  const index = Number(itemButton.dataset.contractIndex);
  if (contractState.selectedIndexes.has(index)) {
    contractState.selectedIndexes.delete(index);
  } else if (contractState.selectedIndexes.size < 10) {
    contractState.selectedIndexes.add(index);
  }
  renderContractItems();
});

document.getElementById('contractSubmit')?.addEventListener('click', async () => {
  await refreshActiveUserFromSupabase();
  const user = getActiveUser();
  const items = getContractItems();
  const indexes = Array.from(contractState.selectedIndexes).sort((a, b) => b - a);
  if (!user || indexes.length < 3 || indexes.length > 10) return;

  const inputValue = indexes.reduce((sum, index) => sum + Number(items[index]?.price || 0), 0);
  const maxReward = inputValue * 3;
  const contractUserChance = Math.max(0, Math.min(100, Number(user.contractPayoutChance ?? 100)));
  const contractSystemEnabled = isPayoutSystemEnabled(user);
  const contractWon = contractSystemEnabled
    ? Math.random() * 100 < contractUserChance
    : true;
  indexes.forEach((index) => {
    const actualIndex = items[index]?.inventoryIndex;
    if (Number.isInteger(actualIndex) && actualIndex >= 0) user.inventory.splice(actualIndex, 1);
  });

  const candidates = availableSkins.filter((item) => Number(item.price || 0) >= inputValue && Number(item.price || 0) <= maxReward);
  const selectedSkin = items[indexes[indexes.length - 1]];
  const reward = candidates.length
    ? candidates.reduce((closest, item) => Number(item.price || 0) > Number(closest?.price || 0) ? item : closest, null)
    : selectedSkin ? {
      name: `${selectedSkin.name} | Контрактный дроп`,
      rarity: selectedSkin.rarity || 'blue',
      image: selectedSkin.image || getSkinImageByRarity(selectedSkin.rarity || 'blue'),
      price: inputValue
    } : null;
  const contractReward = contractWon && reward ? {
    name: reward.name,
    price: Math.min(Number(reward.price || 0), maxReward || Number(reward.price || 0)),
    rarity: reward.rarity || 'blue',
    image: reward.image || getSkinImageByRarity(reward.rarity || 'blue')
  } : null;
  if (contractReward) {
    user.inventory.push(contractReward);
    const rewardPreview = document.getElementById('contractRewardPreview');
    const rewardName = document.getElementById('contractRewardName');
    if (rewardPreview) rewardPreview.innerHTML = `<img src="${contractReward.image}" alt="${contractReward.name}" />`;
    if (rewardName) rewardName.textContent = contractReward.name;
  }
  const resultText = contractReward
    ? `Получен скин: ${contractReward.name}`
    : 'Не удалось создать скин для контракта.';

  user.contracts = Number(user.contracts || 0) + 1;
  user.bestDrop = getBestDropFromInventory(user.inventory) || user.bestDrop;
  saveActiveUser(user);
  const users = getUsers();
  const userIndex = users.findIndex((item) => item.id === user.id || item.username === user.username);
...
async function openUpgradeScreen() {
  ...
  const upgradeItems = getUpgradeInventoryItems();
  selectedUpgradeItemIndex = upgradeItems.length ? pickUpgradeItemByChance(upgradeItems) : -1;
...
function getCanonicalUpgradeChance(value = selectedUpgradeChance) {
  ...
  return Math.max(5, Math.min(100, Number.isFinite(nextValue) ? nextValue : 40));
}
...
function pickUpgradeItemByChance(items) {
  if (!Array.isArray(items) || !items.length) {
    return -1;
  }

  const weights = items.map((item) => 1 / Math.max(1, Number(item.price || 0)));
...
function getUpgradeRewardPreview(selectedItem) {
  ...
  const chance = Math.max(5, Math.min(100, Number(upgradeScreenConfig.chance) || 50));
...
function runUpgradeAnimation() {
  ...
  const user = getActiveUser();
  const payoutChance = isPayoutSystemEnabled(user)
    ? getCanonicalUpgradeChance(user.upgradePayoutChance ?? 40)
    : upgradeScreenConfig.chance;
